...
namespace Web.Controllers.Implements
{
    ...
    public class PedidoController : BaseController<PedidoDto, PedidoSelect, IPedidoService>
    {
        public PedidoController(IPedidoService service, ILogger<PedidoController> logger) : base(service, logger)
        {
        }

        protected override Task<IEnumerable<PedidoSelect>> GetAllAsync()
        {
            return _service.GetAllAsync();
        }
        protected override Task<PedidoSelect?> GetByIdAsync(int id)
        {
            return _service.GetByIdAsync(id);
        }
        protected override Task AddAsync(PedidoDto dto)
        {
            return _service.CreateAsync(dto);
        }

        protected override Task<bool> UpdateAsync(int id, PedidoDto dto)
        {
